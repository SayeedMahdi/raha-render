...
// @access  private
const createChat = asyncHandler(async ({ body, user, t }, res, next) => {
...
const checkTicketStatus = asyncHandler(async ({ user }, res, next) => {
	const clientTicket = await Ticket.findOne({
		client_id: user.id,
		$or: [{ status: "open" }, { status: "in_progress" }],
	})
...
const chatDetails = asyncHandler(async ({ user }, res, next) => {
	const clientTicket = await Ticket.findOne({
		client_id: user.id,
		$or: [{ status: "open" }, { status: "in_progress" }],
	}).populate("messages.user", "fullName image publicId")
